...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // dit is die DELETE
    document.getElementById('deleteKamerForm').addEventListener('submit', async (event) => {
        event.preventDefault();
        const id = document.getElementById('kamer-id-delete').value.trim();

        if (!id) {
            alert("Voer een geldig Kamer ID in.");
            return;
        }
        ...
        try {
            ...
            const messageElement = document.getElementById('deleteKamerMessage');
            if (response.status === 200) {
                messageElement.textContent = `Kamer met ID ${id} is succesvol verwijderd.`;
                messageElement.style.color = "limegreen";
            } else if (response.status === 404) {
                messageElement.textContent = `Kamer met ID ${id} bestaat niet.`;
